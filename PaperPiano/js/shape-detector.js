...
class ShapeDetector {
    ...
    _detectWithPaperIsolation(src, debugCanvas) {
        ...
        // --- Step 3: CLAHE for local contrast normalisation, then blur ---
        // Handles uneven lighting (desk lamp on one side, shadows, etc.)
        // Fallback to equalizeHist if CLAHE not available in this OpenCV.js build
        ...
        if (typeof cv.CLAHE === 'function') {
            ...
            cv.equalizeHist(gray, equalised);
        }

        const blurred = new cv.Mat();
        cv.GaussianBlur(equalised, blurred, new cv.Size(5, 5), 0);
        equalised.delete();

        // --- Step 4: multi-strategy threshold within the masked region ---
        const strategies = [];

        // Strategy A – Adaptive threshold (tuned for thin markers)
        try {
        ...
        // Strategy C – Canny (tighter thresholds to reject paper texture / shadows)
        try {
        ...
        // Best strategy: score by shape count but penalise high size variance
        // (real piano keys are similar-sized; noise produces random sizes)
        strategies.sort((a, b) => {
            const score = (r) => {
                const all = [...r.result.rectangles, ...r.result.circles];
                if (all.length === 0) return 0;
                const areas = all.map(s => s.area);
                const mean = areas.reduce((s, v) => s + v, 0) / areas.length;
                const std = Math.sqrt(areas.reduce((s, v) => s + (v - mean) ** 2, 0) / areas.length);
                const coefVar = mean > 0 ? std / mean : 1;
                // Reward count, penalise variance (cv > 0.8 means very noisy)
                return all.length * Math.max(0.2, 1 - coefVar);
            };
            return score(b) - score(a);
        });

        // Log which strategy won and its score
        if (strategies.length > 0) {
            const winner = strategies[0];
            const all = [...winner.result.rectangles, ...winner.result.circles];
            const areas = all.map(s => s.area);
            const mean = areas.length > 0 ? (areas.reduce((s, v) => s + v, 0) / areas.length) : 0;
            log += ` | Winner: ${all.length} shapes, avg area=${mean.toFixed(0)}`;
        }
    ...
    _findPaper(gray) {
        ...
        // 1. Threshold: use Otsu to adaptively find the paper brightness
        //    (much more robust than a fixed value under varied lighting)
        const bright = new cv.Mat();
        cv.threshold(gray, bright, 0, 255, cv.THRESH_BINARY + cv.THRESH_OTSU);

        // 2. Close small gaps (text, marker lines on the paper)
    ...
    _extractShapes(binaryImg, label, paperCnt, paperMask) {
        ...
        // 1. Dilate pen strokes to bridge gaps in hand-drawn corners,
        //    then close to fill small breaks in closed shapes.
        const dilated = new cv.Mat();
        const dilK = cv.getStructuringElement(cv.MORPH_RECT, new cv.Size(5, 5));
        cv.dilate(binaryImg, dilated, dilK);
        const closeK = cv.getStructuringElement(cv.MORPH_RECT, new cv.Size(3, 3));
        cv.morphologyEx(dilated, dilated, cv.MORPH_CLOSE, closeK);
        closeK.delete();
