...
export class NoteRecognizer {
    ...
    async init(onProgress) {
        try {
            ...
            // Wait for the Tesseract.js <script async> tag to load (max 10s)
            await new Promise((resolve, reject) => {
                if (typeof Tesseract !== 'undefined') { resolve(); return; }
                const t = setTimeout(() => reject(new Error('Tesseract script load timeout')), 10000);
                const poll = () => {
            ...
            // Create the worker with a 10-second timeout so it doesn't hang forever on mobile
            ...
            const timeoutPromise = new Promise((_, reject) =>
                setTimeout(() => reject(new Error('Tesseract worker creation timed out')), 10000)
            );
